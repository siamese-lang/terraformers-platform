...
| GCP runtime/IaC | `infra/terraform/envs/gcp-target-runtime/`, `infra/kubernetes/overlays/gcp-target/`, Vertex target adapters | **IMPLEMENTING M3-R2** | ADR-005 target이 static code/IaC로 구현되었으며 GKE Standard, Workload Identity, Vertex AI, internal OpenSearch target path가 존재한다. 아직 live apply/readiness evidence는 없다. | 동일 target IaC/runtime을 M3-4부터 M9까지 재사용; 별도 evaluation cloud 금지 | existing Terraform/runtime-contract validation, fresh pre-apply quota/billing/model-access evidence, first live apply/readiness smoke |
