...
OUTPUT_DIR="${REPO_ROOT}/artifacts/m2-upload-analysis-baseline"
...
checks=(runtime_ready authenticated_upload project_created source_file_registered analysis_job_created analysis_terminal terraform_validation result_file_registered terraform_inline_readback terraform_checksum_match source_binary_persisted source_byte_readback result_binary_persisted)
...
first_confirmed_gap=NOT_COVERED
...
set_gap() { [[ "${first_confirmed_gap}" != NOT_COVERED ]] || first_confirmed_gap="$1"; }
...
collect_diagnostics() {
  ...
  kubectl -n "${NAMESPACE}" logs deployment/terraformers-backend --tail=-1 >"${OUTPUT_DIR}/backend.log" 2>&1 || true
  {
    ...
    printf 'analysis_status=%s\nsource_image_http_status=%s\nsource_object_http_status=%s\nfirst_confirmed_gap=%s\ncloud_credentials_required=false\n' \
      "${analysis_status}" "${source_image_http_status}" "${source_object_http_status}" "${first_confirmed_gap}"
...
upload_status="$(curl -sS -o "${OUTPUT_DIR}/upload-response.json" -w '%{http_code}' -H "Authorization: Bearer ${owner_token}" -F "file=@${TEMP_DIR}/architecture.png;type=image/png" -F 'projectName=M2 Upload Analysis Baseline' "http://127.0.0.1:${BACKEND_PORT}/api/upload")"
...
api_get "/api/projects/${project_id}" "${OUTPUT_DIR}/project-response.json" >"${OUTPUT_DIR}/project-http-status.txt"
api_get "/api/project-tree/${project_id}" "${OUTPUT_DIR}/project-tree.json" >"${OUTPUT_DIR}/project-tree-http-status.txt"
...
echo '[m2-upload-analysis-baseline] current behavior collected and classified'
